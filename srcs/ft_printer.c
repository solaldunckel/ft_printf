...
/*   Updated: 2019/10/21 17:16:00 by sdunckel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_print_str(const char *s, t_printf *tab)
{
	...
	if (tab->precision)
		tab->tmp = ft_strjoin_len(tab->tmp, cpy, tab->precision_width);
	else
		tab->tmp = ft_strjoin(tab->tmp, cpy);
	if (tab->width > len && tab->minus)
		tab->tmp = ft_strjoin(tab->tmp, ft_print_sp(tab->width - len,
			tab->zero));
...
void	ft_print_add(long unsigned add, t_printf *tab)
{
	...
	tab->precision && add == 0 && tab->precision_width == 0 ? len -= 1 : 0;
	if (tab->width > len && !tab->minus && !tab->zero)
		tab->tmp = ft_print_sp(tab->width - len, tab->zero);
	tab->tmp = ft_strjoin(tab->tmp, x);
	if (tab->width > len && !tab->minus && tab->zero && add != 0)
		tab->tmp = ft_strjoin(tab->tmp, ft_print_sp(tab->width - len,
			tab->zero));
	if (add == 0)
		tab->tmp = ft_strjoin_len(tab->tmp, tab->num, tab->precision_width);
	else
		tab->tmp = ft_strjoin(tab->tmp, tab->num);
	if (tab->width > len && tab->minus)
		tab->tmp = ft_strjoin(tab->tmp, ft_print_sp(tab->width - len,
			tab->zero));
	tab->s = ft_strjoin(tab->s, tab->tmp);
...
void	ft_print_hex(unsigned int hex, int uppercase, t_printf *tab)
{
	...
	{
		tab->zero = 1;
		tab->tmp = ft_strjoin(tab->tmp, ft_print_sp(tab->precision_width - len,
			tab->zero));
		len = tab->width - tab->precision_width;
	}
	tab->zero = 0;
	uppercase ? tab->tmp = ft_strjoin(tab->tmp,
		ft_itoa_hex(hex, "0123456789ABCDEF")) : 0;
	!uppercase ? tab->tmp = ft_strjoin(tab->tmp,
		ft_itoa_hex(hex, "0123456789abcdef")) : 0;
	if (tab->width > len && tab->minus)
		tab->tmp = ft_strjoin(tab->tmp, ft_print_sp(tab->width - len,
